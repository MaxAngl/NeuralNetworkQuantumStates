"""
Script pour tracer le temps d'exécution au champ critique (H ≈ 1) en fonction de L
avec fit polynomial et affichage du R²
"""
...
H_critical = 1.0  # Champ critique
H_tolerance = 0.05  # Tolérance pour sélectionner H ≈ 1
...
# Collecter les temps d'exécution pour H ≈ 1 pour chaque L
L_list = []
time_per_iter_list = []
            ...
            if run_folder.is_dir():
                ...
                if meta_file.exists():
                    with open(meta_file, 'r') as f:
                        ...
                        # Vérifier si H est proche du champ critique
                        h_value = meta.get('hamiltonian', {}).get('h', None)
                        exec_time = meta.get('execution_time_seconds')
                        n_iter = meta.get('n_iter')
                        
                        if (h_value is not None and exec_time is not None and 
                            n_iter is not None and n_iter > 0 and
                            abs(h_value - H_critical) <= H_tolerance):
                            time_per_iter = exec_time / n_iter
                            times_per_iter.append(time_per_iter)
        
        if len(times_per_iter) > 0:
            # Si trop peu de données (< 3 runs), ne pas filtrer
            if len(times_per_iter) < 3:
                times_filtered = times_per_iter
                print(f"L={L}: {len(times_per_iter)} runs (pas de filtrage)")
            # Pour L=100, filtrer pour garder seulement les valeurs autour de 4s/iter
            elif L == 100:
            ...
            if len(times_filtered) > 0:
                ...
                print(f"L={L} à H≈{H_critical}: {len(times_filtered)} runs, temps moyen = {mean_time:.4f} ± {std_time:.4f} s/iter")
            else:
                print(f"L={L}: Aucune donnée valide après filtrage")
        else:
            print(f"L={L}: Aucun run trouvé pour H ≈ {H_critical}")
    else:
...
# Effectuer différents fits
fits = {}

try:
    # Fit sans pondération par les erreurs pour plus de stabilité
    popt_quad, _ = curve_fit(quadratic_fit, L_array, time_array)
...
try:
    popt_power, _ = curve_fit(power_fit, L_array, time_array, p0=[0.001, 2])
...
ax1.set_title(f'Temps d\'exécution au champ critique (H ≈ {H_critical}) vs L\n(échelle linéaire)', 
...
ax2.set_title(f'Temps d\'exécution au champ critique (H ≈ {H_critical}) vs L\n(échelle log-log)', 
             fontsize=16, fontweight='bold', pad=20)
ax2.set_xscale('log')
ax2.set_yscale('log')
...
output_file = "execution_time_at_critical_H.png"
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"\nGraphique sauvegardé : {output_file}")

# Afficher les R² pour chaque fit
print("\n=== Qualité des fits (R²) ===")
