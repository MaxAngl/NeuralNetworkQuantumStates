"""
Comparison: Standard training vs Epoch-based training (mini-batch h0 + data augmentation)
Both use DualEmbed ansatz.

Protocol:
- Standard: all h0 values at once, fixed disorder, 300 iterations
- Epoch: cycle through h0 subsets with fresh disorder each epoch, 300 total iters

Test: 2D grid (h0_test × sigma_test) → colormap of median V-score

Saves to: graphs/epoch_comparison/
"""
...
# Training h0 values (spanning ordered → paramagnetic)
h0_train_list = [0.3, 0.6, 0.8, 1.0, 1.2, 1.5, 2.0, 3.0]
sigma_train = 0.1

# Standard training
n_replicas_per_h0_std = 3          # 8 × 3 = 24 replicas
n_replicas_total = len(h0_train_list) * n_replicas_per_h0_std  # 24
n_iter_total = 300

# Epoch training
n_h0_per_epoch = 4                 # 4 h0 values per mini-epoch
n_reps_per_h0_epoch = n_replicas_total // n_h0_per_epoch  # 6
n_iter_per_epoch = 25
n_epochs = n_iter_total // n_iter_per_epoch  # 12
...
# Test grid
h0_test_list = np.linspace(0.2, 3.5, 14)
...
out_dir = os.path.join(project_root, "graphs", "epoch_comparison")
...
meta = {
    ...
    "sampling": {"chains_per_replica": chains_per_replica, "samples_per_chain": samples_per_chain},
    "test_grid": {
...
def train_epoch():
    ...
    print(f"  {n_epochs} epochs × {n_iter_per_epoch} iters = {n_epochs * n_iter_per_epoch} total")
    print("=" * 60)
    ...
    for epoch in range(n_epochs):
        ...
        print(f"  Epoch {epoch+1:2d}/{n_epochs} | h0={[f'{h:.1f}' for h in h0_subset]}")
...
def evaluate_grid(vs, label):
    """Evaluate trained model on (h0_test × sigma_test) grid."""
    print(f"\n  Evaluating {label} on {len(h0_test_list)}×{len(sigma_test_list)} grid...")
    ...
    pbar = tqdm(total=total, desc=f"  {label}", leave=False)
...
print("=" * 60)
print("  Epoch Training Comparison")
print(f"  L={L}, h0_train={h0_train_list}")
...
grid_std = evaluate_grid(vs_std, "Standard")
grid_ep = evaluate_grid(vs_ep, "Epoch")

# Compute medians
median_std = np.median(grid_std, axis=2)  # (n_h0, n_sigma)
...
# Shared color limits
vmin = min(median_std.min(), median_ep.min())
...
# Mark training h0 values
for h in h0_train_list:
    ax.axhline(h, color="red", alpha=0.3, linewidth=0.8, linestyle="--")
...
ax.set_title("Standard training\n(all h₀ at once, fixed disorder)", fontsize=12)
...
for h in h0_train_list:
    ax2.axhline(h, color="red", alpha=0.3, linewidth=0.8, linestyle="--")
...
ax2.set_title("Epoch training\n(mini-batch h₀ + fresh disorder)", fontsize=12)
plt.colorbar(im2, ax=ax2, label="Median V-score")

# --- Panel 3: Ratio ---
ax3 = axes[2]
ratio = median_ep / median_std
im3 = ax3.pcolormesh(
    sigma_grid, h0_grid, ratio,
    norm=TwoSlopeNorm(vcenter=1.0, vmin=min(0.1, ratio.min()), vmax=max(2.0, ratio.max())),
    cmap="RdBu_r", shading="auto",
)
for h in h0_train_list:
    ax3.axhline(h, color="black", alpha=0.2, linewidth=0.8, linestyle="--")
...
# Global title
fig.suptitle(
    f"Epoch vs Standard Training — DualEmbed ViTFNQS\n"
    f"L={L}, $\\sigma_{{train}}$={sigma_train}, {n_iter_total} iters, "
    f"h₀_train={h0_train_list}\n"
    f"Red dashes = training h₀ values, White dash = $\\sigma_{{train}}$",
    fontsize=13, y=1.02,
)

plt.tight_layout()

pdf_path = os.path.join(out_dir, "epoch_comparison.pdf")
png_path = os.path.join(out_dir, "epoch_comparison.png")
