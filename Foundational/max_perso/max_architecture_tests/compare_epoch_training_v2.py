"""
Comparison: Standard training vs Epoch-based training (v2)
Both use DualEmbed ansatz. MC-based evaluation (no FullSumState).

Protocol:
- Standard: all h0 at once, fixed disorder, 400 iterations
- Epoch: cycle through h0 subsets with fresh disorder each epoch
         8 epochs × 50 iters = 400 total
- Evaluation: MCState sampling (scalable to large L)

Test: 2D grid (h0_test × sigma_test) → colormap of median V-score

Saves to: graphs/epoch_comparison_v2/
"""
...
# Training h0 values — 20 values spanning ordered → paramagnetic
h0_train_list = list(np.round(np.linspace(0.2, 4.0, 20), 2))
sigma_train = 0.1

# Standard training
n_replicas_per_h0_std = 2          # 20 × 2 = 40 replicas
n_replicas_total = len(h0_train_list) * n_replicas_per_h0_std  # 40
n_iter_total = 400

# Epoch training
n_h0_per_epoch = 8                 # 8 of 20 h0 per mini-epoch
n_reps_per_h0_epoch = n_replicas_total // n_h0_per_epoch  # 5
n_iter_per_epoch = 50
n_epochs = n_iter_total // n_iter_per_epoch  # 8
...
# Test grid
h0_test_list = np.linspace(0.1, 4.5, 15)
...
out_dir = os.path.join(project_root, "graphs", "epoch_comparison_v2")
...
def train_standard():
    ...
    print(f"  {len(h0_train_list)} h0 × {n_replicas_per_h0_std} replicas = {n_replicas_total}")
...
def evaluate_grid_mc(vs, label):
    """Evaluate trained model on (h0_test × sigma_test) grid using MC sampling."""
    print(f"\n  Evaluating {label} on {len(h0_test_list)}×{len(sigma_test_list)} grid (MC, {eval_n_samples} samples)...")
    ...
    for i, h0_t in enumerate(h0_test_list):
        for j, sig_t in enumerate(sigma_test_list):
            ...
            for k, pars in enumerate(test_params):
                ...
                # MC evaluation instead of FullSumState
...
print("=" * 60)
print("  Epoch Training Comparison v2 (MC evaluation)")
...
# Global title
fig.suptitle(
    f"Epoch vs Standard Training — DualEmbed ViTFNQS (MC eval)\n"
    f"L={L}, $\\sigma_{{train}}$={sigma_train}, {n_iter_total} iters, "
    f"{len(h0_train_list)} h₀ train values\n"
    f"Red dashes = training h₀, White dash = $\\sigma_{{train}}$",
    fontsize=13, y=1.02,
)

plt.tight_layout()

pdf_path = os.path.join(out_dir, "epoch_comparison_v2.pdf")
png_path = os.path.join(out_dir, "epoch_comparison_v2.png")
