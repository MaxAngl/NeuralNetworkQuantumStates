"""
Comparison: Standard training vs Epoch-based training (v4)
Both use DualEmbed ansatz. MC-based evaluation.

Changes from v3:
- No h0=0 or negatives. Smallest h0 = 0.1
- 10 epochs x 50 iters = 500 total (more diversity)
- Same total iters for both methods

Saves to: graphs/epoch_comparison_v5/
"""
...
# Training h0 values — 20 values, min=0.1, no negatives
h0_train_list = list(np.round(np.linspace(0.1, 4.0, 20), 2))
sigma_train = 0.1

# Total budget: 500 iters for both
n_iter_total = 500
...
n_iter_per_epoch = 50
n_epochs = n_iter_total // n_iter_per_epoch  # 10
...
# Test grid
h0_test_list = np.linspace(0.05, 4.5, 16)
...
out_dir = os.path.join(project_root, "graphs", "epoch_comparison_v5")
...
print("=" * 60)
print("  Epoch Training Comparison v5 (MC eval, h0>=0.1)")
...
pdf_path = os.path.join(out_dir, "epoch_comparison_v5.pdf")
png_path = os.path.join(out_dir, "epoch_comparison_v5.png")
