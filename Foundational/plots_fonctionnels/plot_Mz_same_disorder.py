...
for idx, sigma in enumerate(SIGMA_TEST_LIST):
    if sigma not in results or len(results[sigma]["mean"]) != len(H0_TEST_LIST):
        continue 
...
ax2.plot(valid_sigmas, max_derivatives, marker='s', markersize=6, color='crimson', linewidth=1.5, linestyle='-')
