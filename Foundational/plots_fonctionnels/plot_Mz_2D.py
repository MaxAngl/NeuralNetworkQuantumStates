...
L = meta["L"]
n_spins = L**2  # NOUVELLE CONVENTION 2D
vit_params = meta["vit_config"]

print(f"🔹 Configuration : Grille 2D de {L}x{L} (Total spins = {n_spins})")
print(f"🔹 Tests sur les sigmas : {SIGMA_TEST_LIST}")

hi = nk.hilbert.Spin(0.5, n_spins) # MODIFIÉ
...
# ==========================================
# Mz = (1/N) * sum(Z_i)
Mz_operator = sum(nk.operator.spin.sigmaz(hi, i) for i in range(n_spins)) * (1.0 / n_spins) # MODIFIÉ
# Mz^2 = Mz @ Mz
...
# On crée un faux paramètre adapté à n_spins
dummy_params = np.zeros(n_spins) # MODIFIÉ
...
for sigma in SIGMA_TEST_LIST:
    ...
    for h0 in tqdm(H0_TEST_LIST, desc=f"Balayage h0"):
        # Génération des N réplicas avec la taille n_spins
        configs = np.abs(rng.normal(loc=h0, scale=sigma, size=(N_TEST_PER_H0, n_spins))) # MODIFIÉ
...
ax1.set_title(f"Magnetization order parameter vs Transverse Field (Grid {L}x{L})", fontsize=14)
...
output_file = os.path.join(RUN_DIR, f"mz2_and_derivative_2D_L={L}.pdf")
plt.tight_layout()
