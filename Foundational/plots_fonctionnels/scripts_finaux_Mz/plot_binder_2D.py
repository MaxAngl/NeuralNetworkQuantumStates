"""
Binder cumulant B = 1 - <Mz^4> / (3 * <Mz^2>^2) pour sigma=0 (sans désordre), 2D TFIM.

Chargement depuis is_samples_2D_L{L}_chunk*.npz (L=3,4,5,6,8 — L=10 exclu, IS non terminé).
Pour sigma=0, h_ref = h0 → pas de repondération IS nécessaire.

Usage:
    python plot_binder_2D.py [--output-dir DIR]
"""
import os
import sys
import glob
...
def binder_cumulant(samples):
    ...
    """
    mz  = samples.mean(axis=1)   # (N_samples,)
    mz2 = np.mean(mz ** 2)
    mz4 = np.mean(mz ** 4)
    ...
    for fn in chunk_files:
        ...
        for j in range(len(h0_indices)):
            ...
            if idx not in points:
                h0_val = float(h_ref[j, 0])   # champ uniforme → prendre site 0
...
ax.set_title(r"Binder cumulant — 2D TFIM sans désordre ($\sigma=0$)", fontsize=13)
...
out_pdf = os.path.join(output_dir, "binder_multiL_2D_sigma0.pdf")
out_png = os.path.join(output_dir, "binder_multiL_2D_sigma0.png")
plt.savefig(out_pdf)
