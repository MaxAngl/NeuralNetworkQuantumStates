"""
Comparison of two susceptibility estimators vs disorder sigma (1D, L=25):
...
Data source: mz2_full_data_L=25.npz  (mz2_raw shape: sigma x h0 x configs)
"""
import os
import numpy as np
...
DATA_PATH  = os.path.join(SCRIPT_DIR,
             "../../logs/Trains_finaux_disordered_1D/run_L=25/mz2_full_data_L=25.npz")
OUT_DIR    = os.path.join(SCRIPT_DIR,
             "../../logs/Trains_finaux_disordered_1D/run_L=25")

# ── parameters ───────────────────────────────────────────────────────────────
H0_MIN = 0.5   # window for searching the transition peak
H0_MAX = 1.5
...
print(f"L={L}, sigma_grid={sigma_grid}, mz2_raw shape={mz2_raw.shape}")
