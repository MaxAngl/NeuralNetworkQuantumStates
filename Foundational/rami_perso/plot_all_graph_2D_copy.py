...
# ==========================================
IS_2D = True 
RUN_DIR = r"/users/eleves-a/2024/rami.chagnaud/Documents/logs/2D_FNQS/Run_2D_L10_FNQS"
...
def compute_metrics(params_batch, desc="metrics"):
    ...
    # Vérifie si le système est suffisamment petit pour l'ED
    ...
    for pars in tqdm(params_batch, desc=desc):
        ...
        # 1. Statistiques Monte Carlo (toujours possibles)
        mc_vs = nk.vqs.MCState(sampler=sa_multi, model=_vs.model, variables=_vs.variables, n_samples=2048)
        stats = mc_vs.expect(H)
        r_hats.append(stats.R_hat)
        tau_corrs.append(stats.tau_corr)
        
        # 2. Énergie et V-score
        if exact_possible:
            # Calculs exacts
            ...
            E_val = res_fs.Mean.real
            var_val = res_fs.variance
            rel_errors.append(abs(E_val - E_exact) / abs(E_exact))
        else:
            # Approximations VMC
            E_val = stats.Mean.real
            var_val = stats.variance
            rel_errors.append(np.nan) # Pas d'énergie exacte
            
        v_scores.append(var_val / (E_val**2 + 1e-12))
...
if hi.is_indexable:
    ...
    print("⚠️ Système trop grand : Plot de l'Erreur Relative ignoré.")
...
print("\nðŸ“ˆ Génération du Plot de Convergence (Historique)...")
...
if log_path:
    ...
    if hi.is_indexable and len(train_params) > 0:
    ...
    plt.savefig(out5)
    print(f"âœ… Plot Convergence sauvegardé : {out5}")
    plt.close()
else:
    print("âš ï¸ Pas de fichier log trouvé, plot de convergence ignoré.")

print("\n TERMINE ! Tous les graphiques sont générés.")