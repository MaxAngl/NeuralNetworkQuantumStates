...
import zipfile
import flax
import jax                      # <-- Import obligatoire pour jax.random
...
output_dir = Path("/users/eleves-a/2024/rami.chagnaud/Documents/logs/2D_FNQS/Run_2D_L10_FNQS")
...
print(f"🔹 Système détecté via meta : Grille {L}x{L} | Total spins={n_spins}")

# Utilisation correcte du générateur aléatoire JAX
k = jax.random.key(1) 
hi = nk.hilbert.Spin(0.5, n_spins)
ps = nkf.ParameterSpace(N=1, min=2.5, max=3.5)

# --- RECONSTRUCTION DU MODÈLE ET CHARGEMENT ---
...
checkpoint_path = str(output_dir / "state_200.nk")
...
def create_operator(params):
    ...
    ha_X = sum(nkf.operator.sigmax(hi, i) for i in range(n_spins))
    
    # Interactions 2D avec conditions aux limites périodiques
...
exact_possible = hi.is_indexable
if exact_possible:
    Mz2_mat = (Mz @ Mz).to_sparse()
else:
    print(f"⚠️ Système trop grand (N={n_spins}). Calculs de diagonalisation exacte ignorés.")

# --- 1. DEFINITION DES POINTS ---
h_train = np.linspace(2.5, 3.5, 8)
h_test = np.linspace(1.5, 4.5, 50) 
...
if exact_possible:
    for h_val in tqdm(h_all, desc="Calculs Exacts"):
        _ha = create_operator(jnp.array([h_val]))
...
def get_relative_error(h_vals, vmc_vals, exact_h, exact_vals):
    if not exact_possible:
        return np.full(len(h_vals), np.nan)
    errors = []
...
if exact_possible:
    # Filtrer les NaN pour éviter les erreurs Matplotlib en log-scale
    valid_test = ~np.isnan(err_e_test)
    valid_train = ~np.isnan(err_e_train)
    ax[0, 1].scatter(np.array(vmc_test["h"])[valid_test], err_e_test[valid_test], color='orange', marker='x', s=40, label="Err. Test")
    ax[0, 1].scatter(np.array(vmc_train["h"])[valid_train], err_e_train[valid_train], color='red', marker='o', s=60, edgecolors='black', label="Err. Train")
    ax[0, 1].set_yscale('log')
ax[0, 1].set_ylabel("Erreur Relative")
...
if exact_possible:
    valid_m_test = ~np.isnan(err_m_test)
    valid_m_train = ~np.isnan(err_m_train)
    ax[1, 1].scatter(np.array(vmc_test["h"])[valid_m_test], err_m_test[valid_m_test], color='orange', marker='x', s=40)
    ax[1, 1].scatter(np.array(vmc_train["h"])[valid_m_train], err_m_train[valid_m_train], color='red', marker='o', s=60, edgecolors='black')
    ax[1, 1].set_yscale('log')
ax[1, 1].set_ylabel("Erreur Relative")
ax[1, 1].set_title("Précision de $M_z^2$")

for a in ax.flat:
    a.axvspan(2.5, 3.5, color='gray', alpha=0.1)
    a.set_xlabel("Champ h")
    a.legend()
    a.grid(True, which="both", alpha=0.2)

plt.tight_layout()
plt.savefig(output_dir / "Foundational_extrapolation_test_with_errors_2D.pdf")
print(f"\n✅ Simulation terminée. Graphiques sauvegardés dans {output_dir}")